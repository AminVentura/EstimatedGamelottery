...
// --- FUNCIÓN DE FINALIZACIÓN MEJORADA ---
function finalizeDrawing(date, allNumbers, ranges) {
    ...
    if (!specialNum || mainNumbers.length !== 5 || !validateNumber(specialNum, ranges.special) || !mainNumbers.every(n => validateNumber(n, ranges.main))) {
...
function processCash4LifeBlock(text) {
    // Formato específico para Cash 4 Life
    ...
    for (const line of lines) {
        ...
        // Extraer fecha (formato: Month Day, Year)
        const dateMatch = trimmedLine.match(/(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),\s*(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s*(\d{1,2}),\s*(\d{4})/i);
        if (dateMatch) {
        ...
        const specialMatch = trimmedLine.match(/CB:\s*(\d)/i);
        if (specialMatch) {
            specialNum = parseInt(specialMatch[1]);
        }
        
        // Si tenemos fecha y números, procesamos
        if (date && mainNumbers.length === 5 && specialNum !== null) {
            return { date, mainNumbers, specialNum };
        }
    }
...
// --- FUNCIÓN ESPECÍFICA PARA MEGA MILLIONS ---
function processMegaMillionsBlock(text) {
    // Formato específico para Mega Millions
    const lines = text.split('\n');
    let date = null;
    let mainNumbers = [];
    let specialNum = null;
    ...
    for (const line of lines) {
        ...
        // Extraer fecha (formato: MM/DD/YYYY)
        ...
        const specialMatch = trimmedLine.match(/MEGAPLIER\s*(\d{1,2})/i);
        if (specialMatch) {
            specialNum = parseInt(specialMatch[1]);
        }
...
function parsePastedData(text, lottery) {
    ...
    if (lottery === 'cash4life') {
        // Procesar cada bloque para Cash 4 Life
        ...
        // Procesar cada bloque para Mega Millions
        const blocks = text.split(/\n\n/); // Separar por doble salto de línea
        for (const block of blocks) {
            if (block.trim() === '') continue;
        ...
        // Para Powerball, usamos la lógica anterior
...
function setupDemoMode() {
    const demoPowerballData = [
        { date: '2025-08-25', mainNumbers: JSON.stringify([1, 10, 25, 30, 45]), special: 12 }, 
        { date: '2025-08-22', mainNumbers: JSON.stringify([5, 12, 28, 40, 55]), special: 20 }
    ];
    const demoCash4LifeData = [
        { date: '2025-08-25', mainNumbers: JSON.stringify([2, 5, 10, 15, 20]), special: 3 }, 
        { date: '2025-08-22', mainNumbers: JSON.stringify([8, 12, 20, 35, 40]), special: 1 }
    ];
    const demoMegaMillionsData = [
        { date: '2025-08-25', mainNumbers: JSON.stringify([5, 10, 25, 35, 70]), special: 24 }, 
        { date: '2025-08-22', mainNumbers: JSON.stringify([8, 12, 20, 35, 70]), special: 15 }
    ];
    ...
    document.querySelectorAll('.save-btn, .process-btn, #postCommentBtn').forEach(btn => { 
        btn.disabled = true; 
...
function renderHistory(lottery) {
    ...
    itemsToDisplay.forEach(item => {
        ...
        entryDiv.innerHTML = `
            <span class="text-gray-400 text-sm font-semibold flex-shrink-0">${drawingDate}</span>
    ...
    docs.forEach(doc => {
        ...
        commentDiv.innerHTML = `
            <div class="flex justify-between items-center mb-2">
                <span class="text-sm text-gray-400 font-bold">${comment.userId.substring(0, 8)}...</span>
